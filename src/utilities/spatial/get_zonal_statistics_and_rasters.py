...
# local utilities
from src.utilities.get_data_source_paths_as_dicts import get_data_source_paths_as_dicts

def get_zonal_statistics_and_rasters(sa_scope, variable, year):

    # set data paths
    #variables

    paths = get_data_source_paths_as_dicts()
    climate_var_path = paths['climate_variable_paths'][variable]
    abs_region_path = paths['abs_sa_data_paths'][sa_scope]

    masking_shapefile = gpd.read_file(abs_region_path) #this is the 'gpd' var in Yuan's original script
    data_file = xr.open_dataset(climate_var_path) #this is the 'ds_xxx' file in Yuan's original script

    data = xr.open_dataset(data_file)

    print(f'processed data for ABS {sa_scope} for variable {variable} for year {year}')
